...
class _AmplitudeModulation:
    ...
    def state(self) -> bool:
        if self._parent.socket is None:
            return False
        return bool(int(self._parent.query(f':am:state')))

    @state.setter
    def state(self, new_value: bool) -> None:
        self._parent.issue(f':am:state', bool(new_value))

    @property
    def source(self) -> str:
        if self._parent.socket is None:
            return ''
        return self._parent.query(f':am:source')
    ...
    def source(self, new_value: str) -> None:
        ...
        self._parent.issue(f':am:source', new_value)

    @property
    def depth(self) -> float:
        if self._parent.socket is None:
            return nan
        return float(self._parent.query(f':am:depth'))
    ...
    def depth(self, new_value: float) -> None:
        ...
        self._parent.issue(f':am:depth', new_value)

    @property
    def sensitivity(self) -> float:
        if self._parent.socket is None:
            return nan
        return float(self._parent.query(f':am:sensitivity'))
    ...
    def sensitivity(self, new_value: float) -> None:
        ...
        self._parent.issue(f':am:sensitivity', new_value)

    @property
    def frequency(self) -> float:
        if self._parent.socket is None:
            return nan
        return float(self._parent.query(f':am:internal:frequency'))

    @frequency.setter
    def frequency(self, new_value: float) -> None:
        """ Sets the internal amplitude modulation rate [10..50000 Hz]. """
        if not (10. <= new_value <= 50_000.):
            raise ValueError(f'Invalid Internal AM Frequency: {new_value}')
        self._parent.issue(f':am:internal:frequency', new_value)
...
class _LFOutput:
    ...
    def state(self) -> bool:
        if self._parent.socket is None:
            return False
        return bool(int(self._parent.query(f':lfOutput:state')))

    @state.setter
    def state(self, new_value: bool) -> None:
        """ Activates LF signal output. """
        self._parent.issue(f':lfOutput:state', bool(new_value))

    @property
    def source(self) -> str:
        if self._parent.socket is None:
            return ''
        return self._parent.query(f':lfOutput:source')
    ...
    def source(self, new_value: str) -> None:
        ...
        self._parent.issue(f':lfOutput:source', new_value)
