...
__all__ = ['IVCurveMeasurement', 'iv_curve', 'iv_curve_2', 'iv_curve_of_rate', 'fast_iv_curve']
...
def iv_curve(limits: tuple[float, float], points: int, two_way: bool = False) -> tuple[list[float], list[float]]:
    ...
    def reading_task_callback(_task_idx, _event_type, num_samples, _callback_data):
        ...
        try:
            ...
            setattr(task_adc, 'last_voltage', _data[-1])
        return 0
...
def iv_curve_2(limits: tuple[float, float], points: int, two_way: bool = False) -> tuple[list[float], list[float]]:
    ...
    def reading_task_callback(_task_idx, _event_type, num_samples, _callback_data):
        ...
        try:
            ...
            setattr(task_adc, 'last_current', _data[0, -1])
            setattr(task_adc, 'last_voltage', _data[1, -1])
        return 0
...
def fast_iv_curve(limits: tuple[float, float], points: int, two_way: bool = False) \
    ...
    def reading_task_callback(_task_idx, _event_type, num_samples, _callback_data):
        ...
        setattr(task_adc, 'i', np.concatenate((getattr(task_adc, 'i'), _data[0])))
        setattr(task_adc, 'v', np.concatenate((getattr(task_adc, 'v'), _data[1])))
        return 0
    ...
    print(task_dac.timing.samp_clk_rate, written_points / (t3 - t0 - 1 / 64) / (2 if two_way else 1))
    print(limits_ptp / (t3 - t0) / (2 if two_way else 1))

    i: NDArray[np.float64]
    v: NDArray[np.float64]
    i = np.array(getattr(task_adc, 'i'))
    v = np.array(getattr(task_adc, 'v'))
...
def iv_curve_of_rate(limits: tuple[float, float], current_rate: float, two_way: bool = False, *,
    ...
    with Task() as task_adc, Task() as task_dac:
        ...
        if current_mode == 'linear':
            if two_way:
                i_set = np.concatenate((
                    ...
                    [min_current]
                )) * ballast_resistance * current_divider
                trigger_on_sequence = np.concatenate((
                    [0.0],
                    np.full(2 * points, 2.0 * trigger_trigger),
                    [0.0]
                ))
            else:
                i_set = np.concatenate((
                ...
                trigger_on_sequence = np.concatenate((
                    [0.0],
                    np.full(points, 2.0 * trigger_trigger),
                    [0.0]
                ))
            ...
            if min_current <= 0 and max_current <= 0:
                ...
                i_set = np.concatenate((
                    -np.square(np.linspace(np.sqrt(-min_current), 0.0,
                                           ...
                                           endpoint=True)),
                    np.square(np.linspace(0.0, np.sqrt(max_current),
        ...
        # t0: float = datetime.now().timestamp()
        task_dac.write(np.row_stack((i_set, trigger_on_sequence)), auto_start=True,
